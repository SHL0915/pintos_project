...
load(const char *file_name, void (**eip)(void), void **esp) {
    ...
    for (; ptr < len; ptr++) {
        if (file_name[ptr] == ' ') {
            fname[ptr] = '\0';
            break;
        } else fname[ptr] = file_name[ptr];
    }
    if (ptr == len) fname[ptr] = '\0';
    ...
    int argcnt = 0;
    for (int i = 0; i < len; i++) {
        if (file_name[i] == ' ') argcnt++;
    }
    argcnt++;
    
    char **args = (char **) malloc(sizeof(char *) * argcnt);
    char *now = (char *) malloc(sizeof(char) * (len + 1));
    int nptr = 0, aptr = 0, tot = 0;
    ...
    for (int i = aptr - 1; i >= 0; i--) {
        ...
        tot += nlen;
        *esp -= nlen;
        strlcpy(*esp, args[i], nlen);
        free(args[i]);
        args[i] = *esp;
    }
    
    int word_align = 0;
    if (tot % 4) word_align += (4 - tot % 4);
    *esp -= word_align;
